...
keyboard to run it from: the line types on the left, `edit` `new` `remove` `↑`
`↓` `fold` `link` on the right, each doing what the key printed beside it does.
Tap a line to put the cursor on it, tap it again to write in it.
